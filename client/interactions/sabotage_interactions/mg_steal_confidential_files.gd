class_name MGStealConfidentialFiles
extends MiniGameBase

## Phase 2 Impostor Sabotage Objective: Steal Confidential Files (Executive Office)
## Decrypt executive safe lock and siphon classified Asterion research dossier.
...
var step_names: Array[String] = [
	"1. BYPASS BIOMETRIC TUMBLER LOCK",
	"2. EXTRACT ASTERION PROJECT DOSSIER",
	"3. PURGE TERMINAL ACCESS AUDIT STAMP"
]
...
func _build_ui() -> void:
	ui_elements = MiniGameUIHelper.create_modal_layout(
		self,
		"[RESTRICTED] Executive Safe Infiltration",
		"Steal confidential facility research files without alerting facility security.",
		Vector2(640, 460),
	...
	ui_elements["status_label"].text = "STATUS: SAFE LOCKED. COMMENCE TUMBLER BYPASS."
...
func _on_step_pressed(idx: int) -> void:
	...
	step_buttons[idx].text = "✔ [COMPLETED] " + step_names[idx]
	...
	if current_step < TOTAL_STEPS:
		...
		ui_elements["status_label"].text = "STATUS: EXECUTING ESPIONAGE STEP %d..." % (current_step + 1)
	else:
		ui_elements["status_label"].text = "FILES ACQUIRED. EVIDENCE RESIDUE REMAINS ON SAFE CABINET."
